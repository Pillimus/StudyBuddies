...
import { MOCK_GROUPS, MockGroup, MockMember } from '../../mock/mockData';
...
export default function GroupsPage() {
  ...
  const [selected,     setSelected]     = useState<MockGroup | null>(null);
    ...
    <div className="groups-wrap">
      <div className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="btn-ghost profile-btn" onClick={openProfile}>
      ...
      <div className="groups-body">
        <div className="groups-list">
          ...
          {filteredGroups.map(g => (
            <div
              key={g.id}
              className={`group-row ${selected?.id === g.id ? 'active' : ''}`}
          ...
          {filteredGroups.length === 0 && <div className="groups-empty">No results</div>}
        </div>
          ...
          {selected ? (
            <>
              <div className="group-detail-header">
                <Avatar
                  letter={selected.name[0]} color={selected.color} url={selected.avatarUrl}
                  size={50} square editable={isCreator}
                  onEdit={() => { setEditGroupUrl(selected.avatarUrl); setShowGroupPic(true); }}
                />
                    ...
                    {selected.members.map(m => (
                      <div key={m.username} className="member-row">
                        ...
                        <div className="member-actions">
                          {m.username !== ME && (
                            <button className="icon-btn" title="Message" onClick={() => navigate('/chats')}>
                              <ChatGrad />
                            </button>
                          )}
              ...
              <div className="collapsible">
                ...
                {expanded.events && (
                  <div className="collapsible-body scrollable">
                    {selected.events.length === 0 && <div className="events-empty-msg">No events yet</div>}
                    ...
                    <button className="btn-ghost create-event-btn" onClick={() => navigate('/calendar')}>
                      ＋ Create Event
                    </button>
                  </div>
                )}
      ...
      {showProfile && (
        <div className="modal-overlay" onClick={() => setShowProfile(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <AvatarUploadZone currentUrl={editAvatarUrl} currentColor={profile.avatarColor} letter={avatarLetter} size={88} onFile={setEditAvatarUrl} label="Visible to all group members" />
        ...
        <div className="modal-overlay" onClick={() => setShowLeave(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <p className="leave-confirm-text">
              Are you sure you want to leave <strong>{selected.name}</strong>? You will also be removed from the affiliated group chat.
            </p>
            <div className="modal-footer">
      ...
      {showCreate && (
        <div className="modal-overlay" onClick={() => setShowCreate(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="field create-group-name-field">
              <label>Group Name *</label>
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. COP 4331 Team B" autoFocus />
            </div>
            <div className="field">
              <label>Add Members (usernames, comma separated)</label>
              <input value={newMembers} onChange={e => setNewMembers(e.target.value)} placeholder="e.g. alexr, jordank" />
            </div>
            <div className="modal-footer">
