...
const MOCK_NOTIFICATIONS = [/*MOCK DATA*/
...
export default function Dashboard() {
  ...
  //todo only shows events within next 7 days
    ...
    <div className="dash-wrap">
      ...
      <div className="topbar">
        <div className="topbar-left">
          ...
          <h2 style={{marginLeft:'10px'}}>Hey, <span className="grad-name">{firstName}!</span></h2>
        </div>
        <div className="topbar-right">
          <div style={{position:'relative'}}>
            <button className={`icon-btn ${showBell?'active-btn':''}`} onClick={()=>{setShowBell(!showBell);setShowTodo(false);}} title="Notifications">
              <BellIcon size={17}/>
            </button>
          ...
          <div style={{position:'relative'}}>
            <button className={`icon-btn ${showTodo?'active-btn':''}`} onClick={()=>{setShowTodo(!showTodo);setShowBell(false);}} title="To-do list">
            ...
            <div key={dateStr} className="day-group">
              ...
              {evs.map(event=>(
                <div key={event.id} className="event-row">
                  <span className="event-type-icon" style={{color:TYPE_COLOR[event.type]}}>{TYPE_ICON[event.type]}</span>
                  <div className="event-info" onClick={()=>setSelectedEvent(event)} style={{cursor:'pointer',flex:1}}>
                    <div className="event-title">{event.title}</div>
                    {event.for!=='Me'&&<div className="event-group">{event.for}</div>}
                  </div>
                  <div className="event-time">{fmt12(event.startTime)}</div>
                  <button className="done-chip" onClick={()=>markDone(event.id)}>✓ Mark As Done</button>
                </div>
              ))}
            </div>
          ))}
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                <span style={{fontSize:'20px',color:TYPE_COLOR[selectedEvent.type]}}>{TYPE_ICON[selectedEvent.type]}</span>
            ...
            <div className="event-detail-body">
              {[
                ...
                <div key={i} className="event-detail-row">
                  <span className="detail-label">{r.label}</span><span>{r.value}</span>
                </div>
              ))}
            </div>
            <div className="modal-footer">
              <button className="btn-ghost danger-ghost" onClick={()=>{markDone(selectedEvent.id);setSelectedEvent(null);}}>✓ Mark Done</button>
              <button className="btn-ghost" onClick={()=>{setSelectedEvent(null);navigate('/calendar');}}>View in Calendar</button>
