...
const MOCK_GROUPS = ['COP 4331','ML Study','Calculus 3','Chem Group'];
...
export default function CalendarPage() {
  ...
  function openEdit(ev:AppEvent) {
    setForm({
      ...
      type:ev.type, for:ev.for, description:ev.description, location:ev.location,
    });
    setEditMode(true);
    setSelected(null);
    setShowCreate(true);
        ...
        <div className="modal-overlay" onClick={()=>setShowCreate(false)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            ...
            <div className="field-row">
              ...
              <div className="field"><label>For</label>
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                <span style={{fontSize:'20px',color:TYPE_COLOR[selected.type]}}>{TYPE_ICON[selected.type]}</span>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {[
                ...
                <div key={i} style={{display:'flex',gap:'16px',alignItems:'baseline'}}>
                  <span style={{fontSize:'0.71rem',fontWeight:600,letterSpacing:'0.09em',textTransform:'uppercase',color:'var(--text-muted)',width:'68px',flexShrink:0}}>{r.label}</span>
                  <span style={{color:'var(--text)',fontSize:'0.9rem'}}>{r.value}</span>
                </div>
              ))}
            </div>
            <div className="modal-footer">
              <button className="btn-ghost" style={{color:'var(--teal)',borderColor:'rgba(45,212,212,0.3)'}}
                onClick={()=>{markDone(selected.id);setSelected(null);}}>
                ✓ Mark Done
              </button>
