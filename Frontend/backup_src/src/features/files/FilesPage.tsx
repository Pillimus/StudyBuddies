...
interface FileItem { id:number; name:string; type:string; size:string; group:string|null; uploaded:string; content?:string; }

const MOCK_FILES:FileItem[] = [ /*MOCK DATA*/
...
export default function FilesPage() {
  ...
  function sendAi() { /*INCLUDES MOCK DATA*/
    ...
    <div className="files-wrap">
      ...
      <div className="files-body">
        <div className="files-main page-scroll">
          <div className="files-filters">
            {GROUPS.map(g=><button key={g} className={`filter-tab ${filter===g?'active':''}`} onClick={()=>setFilter(g)}>{g}</button>)}
          </div>
          ...
          {aiOpen&&(
            <div className="ai-body">
              <div className="ai-files-section">
                ...
                {aiFiles.length===0?<div className="ai-no-files">Click ✦ on a file to add it</div>:(
              ...
              <div className="ai-input-row">
                ...
                <button className="btn-primary" onClick={sendAi} style={{padding:'9px 14px',flexShrink:0}}>➤</button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="modal-box file-read-modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                <span style={{fontSize:'20px'}}>{FILE_ICONS[readFile.type]||'📁'}</span>
                <h3>{readFile.name}</h3>
              </div>
            ...
            <div className="file-content-area">
              {readFile.content
                ...
                : <div style={{color:'var(--text-muted)',fontStyle:'italic',textAlign:'center',padding:'40px'}}>
                    File preview not available — connect the backend to read real files.
                  </div>
              }
            </div>
      ...
      {showUpload&&(
        <div className="modal-overlay" onClick={()=>setShowUpload(false)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><h3>Upload File</h3><button className="icon-btn" onClick={()=>setShowUpload(false)}>✕</button></div>
            ...
            <div className="field" style={{marginTop:'16px'}}><label>Associate with Group</label>
      ...
      {shareTarget&&(
        <div className="modal-overlay" onClick={()=>setShareTarget(null)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><h3>Share File</h3><button className="icon-btn" onClick={()=>setShareTarget(null)}>✕</button></div>
            <p style={{fontSize:'0.85rem',color:'var(--text-dim)',marginBottom:'16px'}}>Sharing: <strong>{shareTarget.name}</strong></p>
            <div className="field"><label>Share with (username or group)</label>
